...
unsigned char shouldeat(const char *message)
{
	...
	if (strcasestr(message, "corpse"))
	{
		...
		if (!(strcasestr(message, "human") || strcasestr(message, "were") || strcasestr(message, "kobold") || strcasestr(message, "bee") || strcasestr(message, "bat") || strcasestr(message, "trice") || strcasestr(message, "nymph") || strcasestr(message, "leprechaun") || strcasestr(message, "watch") || strcasestr(message, "soldier") || strcasestr(message, "lieutenant") || strcasestr(message, "captain") || strcasestr(message, "giant spider") || strcasestr(message, "scorpion") || strcasestr(message, "centipede") || strcasestr(message, "yellow mold") || strcasestr(message, "violet fungus") || strcasestr(message, "dog") || strcasestr(message, "cat") || strcasestr(message, "kitten")))
		{
...
const char *chooseaction()
{
	...
	for (pass = 0; pass < 7; pass++)
	{
		...
		if (pass == 0 && hp <= maxhp / 2 && elberethtimer <= 0 && humancount == 1 && !blind)
		{
		...
		else if (pass == 0 && hp <= maxhp / 2 && myx == elberethx && myy == elberethy && elberethtimer >= 8 && humancount == 1 && !blind)
		{
			fprintf(logfile, "reiterating elbereth\n");
			strcpy(action, "E-y Elbereth\r");
			return action;
		}
		else if (pass == 0 && hp <= maxhp / 4 * 3 && myx == elberethx && myy == elberethy && humancount == 1 && !blind)
		{
			fprintf(logfile, "resting on elbereth\n");
			strcpy(action, "s");
			return action;
		}
		else if (pass == 0 && hp <= maxhp / 5 * 3)
		{
		}
		else if (pass > 0 && hp <= maxhp / 5 * 3)
		{
...
void preprocess()
{
	...
	for (idx = 80; idx < 80 * 21; idx++)
	{
		...
		if (canbepeaceful(framebuffer[idx]))
		{
			...
			else if (strcasestr(buffer, "moomaster") || strcasestr(buffer, "dragoniz3r"))
			{
				myx = idx % 80;
				myy = idx / 80;
				//goto startover;
