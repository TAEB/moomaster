...
/* Used to implement replay functionality.  XXX: perhaps split this
 * growing independant part off?
 */
void Write(int fd, const char *message)
{
	if (!replaying) write(fd, message, strlen(message));
}
...
int Read(char *buffer, int len)
{
	...
	if (replaying)
	{
		fread(&ret, sizeof(int), 1, replayfile);
		fread(buffer, sizeof(char), ret < 0 ? 0 : ret, replayfile);
	}
	else
	{
		ret = read(fd2[0], buffer, len);
		fwrite(&ret, sizeof(int), 1, replayfile);
		fwrite(buffer, sizeof(char), ret < 0 ? 0 : ret, replayfile);
	}
	return ret;
}
...
int Time()
{
	...
	if (replaying)
	{
		fread(&val, sizeof(int), 1, replayfile);
	}
	else
	{
		val = time(NULL);
		fwrite(&val, sizeof(int), 1, replayfile);
	}
...
void handlemessage(const char *message, const char *when)
{
	...
	if (strcasestr(message, "see here") || strcasestr(message, "things that are here") || strcasestr(message, "several items here"))
	{
		...
		{
			fprintf(logfile, "Nothing to see here, moving on\n");
			squelchbuffer[look_y*80 + look_x] = '@';
		}
		//sprintf(buffer, "\x12");
		//Write(fd1[1], buffer);
	}
	...
	if (strcasestr(message, "Dlvl:"))
	{
		...
		if (idx != mydlvl)
		{
			...
			memset(trafficbuffer, 0x00, 80 * 25 * 4);
			memset(notebuffer, 0x00, 80 * 25 * 4);
			memset(squelchbuffer, 0x00, sizeof squelchbuffer);
			pendingupstair = 0;
			turncount = 0;
		}
		mydlvl = idx;
...
const char *chooseaction()
{
	...
	for (pass = 0; pass < 7; pass++)
	{
		...
		for (idx = 80; idx < 80 * 22; idx++)
		{
			...
			/* Ignore squelched piles */
			if (squelchbuffer[idx] != 0)
			{
				if (squelchbuffer[idx] == target)
				{
					//fprintf(logfile, "Ignoring squelched pile at %i.%i\n", tx, ty);
					continue;
				}
				else
				{
					fprintf(logfile, "Squelched pile at %i.%i has obviously changed!\n", tx, ty);
					squelchbuffer[idx] = 0;
				}
			}
	...
	/* Save the destination for use by the looking code */
	look_x = tx;
	look_y = ty;
...
void preprocess()
{
	...
	for (idx = 80; idx < 80 * 21; idx++)
	{
		...
		if (canbepeaceful(framebuffer[idx]))
		{
			...
			else if (strcasestr(buffer, "sorearmoo"))
			{
				myx = idx % 80;
				myy = idx / 80;
				//goto startover;
...
void dump_squelch()
{
	...
	squelchout = fopen("squelch.txt", "wb");
	if (!squelchout)
	{
		fprintf(logfile, "Can't open squelch.txt\n");
		return;
	}
	...
	for (idx = 0; idx < 80 * 25; idx++)
	{
		...
		{
			fprintf(squelchout, "%c", squelchbuffer[idx]);
		}
		if (idx % 80 == 79)
		{
			fprintf(squelchout, "\n");
...
int main(int argc, char *argv[])
{
	...
	replaying = (argc == 2 && !strcmp(argv[1], "-replay"));
	replayfile = fopen("replay.dat", replaying ? "rb" : "wb");
	...
	refreshtimer = Time() + REFRESH_TIMER;
	inventorytimer = Time() + INVENTORY_TIMER;

	pid = replaying ? 1 : fork();
	if (pid)
	{
		...
		while (1)
		{
			...
			dump_traffic();
			dump_squelch();
			if (!replaying) fflush(replayfile);
			...
			/* Hack - if a square marked as squelch-@ is adjacent to us, fill in the
			 * correct squelch now.  This is needed because we can't normally see what
			 * we are standing on.
			 */
			...
			for (dx = -1; dx <= 1; dx++)
			{
				for (dy = -1; dy <= 1; dy++)
				{
					idx = (myy + dy) * 80 + (myx + dx);

					if (squelchbuffer[idx] == '@')
					{
						squelchbuffer[idx] = framebuffer[idx];
					}
				}
			}
