		...
		if (frame[idx] == 0x1B)
		{
			...
			for (idx = idx; idx < strlen(frame); idx++)
			{
				if (isalpha(frame[idx]))
				{
					...
					memcpy(token, &frame[idx2], idx - idx2);
					token[idx - idx2 + 1] = '\0';
...
const char *chooseaction()
{
	...
	for (pass = 0; pass < 7; pass++)
	{
		...
		for (idx = 80; idx < 80 * 22; idx++)
		{
			...
			else if (notebuffer[idx] & NOTE_DOOR && pass == 3 && pathto(tx, ty, '+', 1) != '!' && !lycanthropy && trafficbuffer[idx] < traffictimeout() / 2)
			{
				x = idx % 80;
				y = idx / 80;
			...
			else if (isgooditem(framebuffer[idx]) && pass == 4 && pathto(tx, ty, framebuffer[idx], 1) != '!' && trafficbuffer[idx] < traffictimeout() / 2)
			{
				x = idx % 80;
				y = idx / 80;
...
int main(int argc, char *argv[])
{
	...
	if (pid)
	{
		...
		while (1)
		{
			...
			if (myx != cx || myy != cy)
			{
				fprintf(logfile, "Cursor is at %i.%i, last known pos is %i.%i, @ is at %i.%i, %i @ in view\n", cx, cy, lastmyx, lastmyy, myx, myy, count);
