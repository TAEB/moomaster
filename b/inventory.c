...
int getweapontype(const char *message)
{
	...
	else if (strcasestr(message, "long sword"))
	{
		fprintf(logfile, "IDed longsword\n");
		weapontype = WEAPON_LONGSWORD;
	}
	else if (strcasestr(message, "scimitar"))
	{
		weapontype = WEAPON_SCIMITAR;
	}
...
{
	return ((getweapontype(message) == WEAPON_DAGGER && countitems(ITEM_WEAPON, WEAPON_DAGGER) < 10) || getweapontype(message) == WEAPON_LONGSWORD || (!getitem(ITEM_WEAPON, WEAPON_LONGSWORD) && getweapontype(message) == WEAPON_SCIMITAR));
}
...
{
	if (!strstr(message, "Call a ") || !strstr(message, "potion:"))
	{
		return 0;
	}
...
int getscrolltype(const char *message)
{
	if (!strcasestr(message, "scroll"))
	{
		return SCROLL_NONE;
	}
	if (strcasestr(message, "teleportation"))
	{
		return SCROLL_TELEPORTATION;
	}
...
void str2item(const char *line, item_t *item)
{
	...
	tmp = getweapontype(line);
	if (tmp != WEAPON_NONE)
	{
		item->type = ITEM_WEAPON;
		item->subtype = tmp;
	}
	tmp = getscrolltype(line);
	if (tmp != SCROLL_NONE)
	{
...
void refreshinventory()
{
	...
	for (idx = 0; idx < count; idx++)
	{
		...
		else if (strcasestr(inventorystring[idx], "weapon in hand"))
		{
			haveweapon = 1;
		}
	...
	for (idx = 0; idx < count; idx++)
	{
		...
		else if (inventory[idx].type == ITEM_WEAPON && !haveweapon)
		{
			sprintf(buffer, "w%c  ", inventory[idx].letter);
			Write(fd1[1], buffer);
		}
