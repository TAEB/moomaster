...
unsigned char impassable(char c, int x, int y)
{
	...
	return (c == '|' || c == '-' || c == '0' || c == '+' || c == PEACEFUL_MONSTER || c == NOTOUCH_MONSTER || notebuffer[y * 80 + x] & NOTE_BADFLOOR);
}

unsigned char avoidabletrap(int x, int y)
{
	int count = 0;

	if (notebuffer[y * 80 + x + 1] & NOTE_SEENFLOOR)
	{
		count++;
	}
	if (notebuffer[y * 80 + x - 1] & NOTE_SEENFLOOR)
	...
	if (notebuffer[(y + 1) * 80 + x] & NOTE_SEENFLOOR)
	{
		count++;
	}
	if (notebuffer[(y - 1) * 80 + x] & NOTE_SEENFLOOR)
...
int findunexploredarea(int *x, int *y, char *target)
{
	...
	{
		if ((framebuffer[idx] == '.') && (framebuffer[idx + 1] == ' ' || framebuffer[idx - 1] == ' ' || framebuffer[idx + 80] == ' ' || framebuffer[idx - 80] == ' ' || framebuffer[idx + 79] == ' ' || framebuffer[idx + 81] == ' ' || framebuffer[idx - 81] == ' ' || framebuffer[idx - 79] == ' ') && pathto(idx % 80, idx / 80, framebuffer[idx], 1) != '!')
		{
...
int makescore(int dx, int dy, int x, int y, char target)
{
	...
	if (notebuffer[y*80 + x] & NOTE_TRAP)
		score += 20;
	score++;
...
{
	spawnnode(x - 1, y + 1, '1', parent, target, targetx, targety, x, y, test);
	spawnnode(x, y + 1, '2', parent, target, targetx, targety, x, y, test);
	spawnnode(x + 1, y + 1, '3', parent, target, targetx, targety, x, y, test);
	spawnnode(x - 1, y, '4', parent, target, targetx, targety, x, y, test);
	spawnnode(x + 1, y, '6', parent, target, targetx, targety, x, y, test);
	spawnnode(x - 1, y - 1, '7', parent, target, targetx, targety, x, y, test);
	spawnnode(x, y - 1, '8', parent, target, targetx, targety, x, y, test);
	spawnnode(x + 1, y - 1, '9', parent, target, targetx, targety, x, y, test);
}
